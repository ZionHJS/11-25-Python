...
7. open vscode to `server` folder: `code .`
...
9. Open new integrated terminal, should show `(env)` in the terminal
    - if integrated terminal doesn't show `(env)`, press the plus button to open new integrated terminal
10. Install dependencies from your integrated terminal that shows `(env)` so these will be installed only for this project
    - `pip install Flask`
11. create `app.py` at root project level
    - ``` py
      ...
      if __name__ == "__main__":
          app.run(debug=True)
        ```
12. Create `templates` folder with `index.html` inside
...
# Debugging Notes
## Starting server: `python app.py` - when you don't need to pause on breakpoints
